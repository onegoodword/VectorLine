...
function buildSidebar(): void {
  ...
  const lineSection = section('Line', [
    ...
    note(
      'This mode is fill-based; in plotter mode it falls back to uniform-width centerline strokes.',
      () => g().line.plotterMode && !MODULATORS[g().line.mode].plotterCapable,
    ),
    ...
    slider('Thickness steps', { min: 2, max: 50, step: 1 }, () => g().line.steps, (v) => store.set('line', 'steps', Math.round(v))),
